	...
	return(el, binding) => {
		let time = null el.addEventListener("touchstart", (event) => {
			time = setTimeout(() => {
					...
					let tip = binding.value.tip
					if(!flag) {
						let deleteFunction = binding.arg vm[deleteFunction](type, id, index)
					} else {
						vm.$alert(tip)
					}
			...
			el.addEventListener("touchstart", (event) => {
				...
				if(!(binding.value instanceof Array)) {
					...
					if(!(binding.value[0] instanceof Vue && binding.value[0].constructor != Vue)) {
						...
						let params = binding.value.slice(1) let myFunction = binding.arg binding.value[0][myFunction](event, params)
					}
				}
			});
		}
